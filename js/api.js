...
const API = (() => {
  ...
  async function addCafeRow(row) {
    return post('카페_월요약', { action: 'append', row });
  }

  async function updateCafeRow(rowIndex, row) {
    return post('카페_월요약', { action: 'update', rowIndex, row });
  }

  async function deleteCafeRow(rowIndex) {
    return post('카페_월요약', { action: 'delete', rowIndex });
  }

  // ── AI 비용 ────────────────────────────────────────────────
  async function getAICost(year, month) {
    return get('AI_비용', { year, month });
  }

  async function upsertAICost(row) {
    return post('AI_비용', { action: 'upsert', row });
  }

  // ── 알고케어 ───────────────────────────────────────────────
  async function getAlgocare(year, month) {
    return get('알고케어', { year, month });
  }

  // ── 슬랙/오피스키퍼 ────────────────────────────────────────
  async function getSlack(year, quarter) {
    return get('슬랙_오피스키퍼', { year, quarter });
  }

  // ── 권한 ───────────────────────────────────────────────────
  async function getPermission(email) {
    return get('사용자_권한', { email }, false);
  }
